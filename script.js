...
async function carregarNomeGestor() {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/gestor/nome');
...
function ativarMenu() {
    ...
    if (activeLink) {
        document.querySelectorAll('.nav-menu li.active').forEach(item => item.classList.remove('active'));
...
async function carregarGraficoPerformance() {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/equipamentos/status');
...
async function carregarDadosGestor() {
    ...
    try {
        ...
        document.getElementById('profile-name').textContent = dados.nome || 'Nome do Gestor';
        document.getElementById('profile-role').textContent = dados.funcao || 'Gerente de Operações';
    } catch (error) {
...
async function handleAtualizarGestor(event) {
    ...
    try {
        const response = await fetch('http://localhost:3000/api/gestor/atualizar', {
            method: 'PUT',
            headers: {
